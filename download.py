...
if __name__ == '__main__':
    ...
    #permanent_links_url = 'http://dl.dropbox.com/u/214296984/RH_classes/%s/links.txt' % class_id
    #links_url = 'http://sdwa.appliomics.com/u/214296984/RH_classes/%s/links.txt' % class_id
    ...
    modified_targets = [ 
            re.sub("https://", "http://",
                re.sub("dl.dropbox.com", "sdwa.appliomics.com", t, 1)
            )
            for t in targets ]
    mapper = Pool(10).imap if sys.version_info < (2, 7) else Pool(10).imap_unordered
    #mapper = map
    for line in mapper(download, modified_targets):
        print line
    etags.close()
